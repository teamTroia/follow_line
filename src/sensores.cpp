...
const int sensorArrayPin[6] = {
    PB1, PA6, PA5, PA4, PA3, PA0};  // matriz com os nossos 8 sensores de linha
const int sensorBordaPin[2] = {PB10, PA8};

const float sensorArrayErroConst[6] = {
    -4, -2, -1, 1, 2, 4};  // posição dos sensores varia de 3 a -3
...
// int sensorBordaThreshold[2] = {300, 100}; //Valor de limiar dos sensores de
// borda
...
// 0bs: maior valor lido= preto; menor valor lido=branco; abaixo de 3100 é
// branco
int maiorArrayAnalog[6] =
    {4200, 4200, 4200, 4200,
     4200, 4200};  // Maior valor medido pelo array de sensores (valores já
                   // pré-determinados para evitar erros na falta da calibração)
int menorArrayAnalog[6] = {
    1500, 1500, 1500,
    1500, 1500, 1500};  // Menor valor medido pelo array de sensores
int superiorThreshold[6] = {
    3685, 3688, 3721, 3750,
    3772, 3766};  // Maiores valores reais lidos e tirados através de testes
int inferiorThreshold[6] = {
    3150, 3166, 3239,
    3302, 3364, 3343};  // s valores reais lidos e tirados através de testes
...
void Sensores::sensorCalibrate() {
    ...
    if (Calibrado == 0) {
        for (int i = 0; i < 6;
             i++) {  // "zerando" valores dos sensores para adquirir novos sem
                     // limitações, apenas na primeira calibração,
            maiorArrayAnalog[i] =
                0;  // nas seguintes o valor será apenas atualizado
    ...
    for (int j = 0; j < 4000; j++) {
        ...
        if (j % 80 == 0) {
            digitalWrite(
                LED_L3,
                !digitalRead(LED_L3));  // LED piscando durante a calibração
        }
    }
    ...
    for (int L = 0; L < 6; L++) {
        faixaLeituraArray[L] =
            maiorArrayAnalog[L] -
            menorArrayAnalog[L];  // definição da faixa de valores colhidos
        faixaAtuacaoArray[L] =
            (faixaLeituraArray[L]) /
            divisor;  // divisão da faixa para definição de limites
        superiorThreshold[L] =
            maiorArrayAnalog[L] -
            (faixaAtuacaoArray[L]);  // definição de limite superior (preto)
        inferiorThreshold[L] =
            menorArrayAnalog[L] +
            (4 *
             faixaAtuacaoArray[L]);  // definição de limite inferior (branco)
    }
    for (int H = 0; H < 2; H++) {
        faixaLeituraBorda[H] = maiorBordaAnalog[H] - menorBordaAnalog[H];
        faixaAtuacaoBorda[H] = (faixaLeituraBorda[H]) / divisor;
        superiorThresholdBorda[H] =
            maiorArrayAnalog[H] - (faixaAtuacaoBorda[H]);
        inferiorThresholdBorda[H] =
            menorArrayAnalog[H] + (4 * faixaAtuacaoBorda[H]);
    }
...
void Sensores::sensorLer(float &sensorArrayErro, int sensorBordaDig[],
    ...
    for (sensorNum = 0; sensorNum < 6; sensorNum++) {
        sensorArrayAnalog[sensorNum] = analogRead(sensorArrayPin[sensorNum]);
    ...
    pinMode(sensorBordaPin[0], INPUT);
    pinMode(sensorBordaPin[1], INPUT);
    while ((sensorBordaAnalog[0] == 0 || sensorBordaAnalog[1] == 0) &&
           (micros() - auxTemp) < BORDA_RC_TIMEOUT) {
    ...
    if (sensorBordaAnalog[0] == 0)
        sensorBordaAnalog[0] = 65535;  // TA FALANDO QUE SE A LEITURA FOR FALSA,
                                       // TA NO PRETO; 65535 é o PWM em binário
