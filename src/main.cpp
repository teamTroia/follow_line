...
double tempoLeituraTrecho = 0;
void setup() {
  /*afio_remap(AFIO_REMAP_TIM3_PARTIAL);// tem q add isso no código tbm, no caso
  da PB5 afio_remap(AFIO_REMAP_TIM2_PARTIAL_2);//PB11*/
  motor.motorInit();
...
void loop() {
  ...
  */
  ...
  if (sensorBordaDig[0]) {
    if (!sensorBordaDig[1]) {
      if ((micros() - tempoLeituraTrecho) >= 20) {
        Trecho++;
  ...
  */
  ...
  // ---------------------------------------------------------------------------------------------------------------------------------------------------------
  if (Iniciado != 0 &&
      parou == 0) {  // Logo depois de apertarbluetooth.println o botão de Start
    ...
    if (micros() - T_Sen_0 >= 2000) {
      ...
      // setamos uma velocidade pra frente e uma velocidade pra trás q vai fazer
      // o robô rodar no proprio eixo (PID)

      U[0] =
          VELs - VELerro * (abs(ErSen) /
                            3);  // Cálculo da velocidade linear; abs=funçao de
                                 // módulo e dividimos por 3
                                 // por causa da posição dos sensores (3  a -3)

      // qunado o erro for muito grande, ele reduz a velocidade pra não ir muito
      // rápido
      ...
      // parte do integrador, calcula o erro pra parte do integrador, se o erro
      // for maior que 100, ele satura a 100, e limita o erro.
      ...
      // calculo do PID, U[1] é o PID, kp(constante proporcional), KI(integral),
      // KD (derivativa), ai ele depende das variáveis q estão setadas tudo q é
      // setado é dividido por delta T (por isso q tem q calcular o delta T),
      // porque precisamos achar a derivada do erro.

      U[1] = KPs * ErSen + KIs * ErSenInt +
             KDs * (ErSen - ErSen_0) /
                 dT_Sen;  // Velocidade angular calculada pelo PID

      // depois calculamos quanto cada roda tem q rodar, porque uma roda precisa
      // rodar no sentido horario e a outra no sentido anti-horario U[1] é pra
      // frente, U[0] é pra tras, Uv é a velocidade da roda
      ...
      motor.motorSetVel(
          Uv[0] * 100,
          Uv[1] * 100);  // usamos 65535, pq o pwm é de 0 a 2^10-1, ou seja é de
                         // 0 a 65355(eu usei o pwm como 125) no caso
                         // porque o Uv foi calcula pra ser um valor entre 0 e
                         // 1; quebrado (double)
    }
  } else {  // É para as vezes pares que acionar o botão de Start, o robô para e
            // zera a contagem dos sensores de borda
